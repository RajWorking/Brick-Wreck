...
class Game:
    ...
    def game_loop(self):
        ...
        while self.lives:
            ...
            if self.ball.y >= config.HEIGHT:
                ...
                print('\033[K ', self.ball.x, self.ball.y, self.ball.direction, self.ball.speed)
                if config.DEBUG:
                    with open("debug_print/ball_path.txt", "a") as f:
                        print(self.ball.x, self.ball.y, self.ball.direction, file=f)
                self.draw(self.ball)
    ...
    def check_collide(self):
        ...
        # Paddle
        if self.ball.y >= self.paddle.y - 1 and self.ball.x in range(self.paddle.x,
                                                                     self.paddle.x + self.paddle.shape[0]):
            self.ball.reflect(1)  # Vertical
            ...
            if (dir := self.ball.path_cut(brick)) != -1:
                ...
                if config.DEBUG:
                    with open("debug_print/brick_collide.txt", "a") as f:
                        print(brick.x, brick.y, brick.level, file=f)
